...
type TableRow = (
  | string
  | { text: string; bold?: boolean; align?: "left" | "center" | "right" }
)[];
  ...
  rows.forEach((row) => {
    ...
    row.forEach((cell, i) => {
      ...
      doc
        ...
        .text(cellText, startX + 4, startY + 4, {
          width: columnWidths[i] - 8,
          align,
        });
...
export function drawTableRowWithBorders(
  ...
  if (values.length === 0 && labels.length === 1) {
    ...
    doc.font("Helvetica-Bold").text(label, MARGIN_X, yStart, {
  ...
  for (let i = 0; i < labels.length; i++) {
    ...
    doc.font("Helvetica-Bold").text(label, currentX + TEXT_PADDING_X, yStart, {
    ...
    doc.font("Helvetica").text(value, valueXStart, yStart, {
    ...
    if (i < labels.length - 1) {
      doc
        .moveTo(currentX + cellWidth, topEdge)
        .lineTo(currentX + cellWidth, bottomEdge)
        .stroke();
    }
...
export const createGeneralApiTable1 = (
  doc: InstanceType<typeof PDFDocument>,
  policy: FullPolicy,
  order: FullOrder
) => {
  ...
  if (productName.includes("selfcare")) {
    ...
    policyTypeValue = "Lifestyle Care";
  } else if (productName.includes("homecare")) {
    policyTypeValue = "Homecare";
    ...
    if (
      (customerData && customerData.passport_no == null) ||
    ...
    policyTypeValue = "Family HealthCare";
  } else if (productName.includes("parents")) {
    policyTypeValue = "ParentsCare";
    ...
    policyTypeValue = "Individual Health Plan";
  } else if (productName.includes("fbl-takaful health family cover")) {
    policyTypeValue = "Takaful Family Health Cover";
  } else if (productName.includes("fbl-takaful health cover")) {
    policyTypeValue = "Takaful Health Cover";
  } else if (productName.includes("fbl-personal accident")) {
    policyTypeValue = "Accident Takaful";
  } else if (productName.includes("hbl")) {
    policyTypeValue = "Hospital Daily Cash";
  } else if (productName.includes("shifa-daily")) {
    policyTypeValue = "SHIFA-Daily Cash Insurance";
  } else if (productName.includes("scb")) {
    policyTypeValue = "Hospital Takaful Cover";
  } else if (productName.includes("sehat plan")) {
    policyTypeValue = "Comprehensive Health Cover";
  ...
  doc
    .moveTo(15, yStart - 5)
    .lineTo(doc.page.width - 15, yStart - 5)
  ...
  drawTableRow(
    doc,
    yStart,
    [policyNumber, placeOfIssue],
    [policy.policy_code || "123456789", "Karachi"],
    [250, 250]
  );
  drawTableRow(
    doc,
    yStart + 15,
    [issueDate, `${policyType} Type`],
    [format(new Date(policy.issue_date), DATE_FORMAT), policyTypeValue],
    [250, 250]
  );

  doc
    .moveTo(doc.page.width - 15, yStart - 5)
    .lineTo(doc.page.width - 15, yStart + 27)
    .stroke(); // vertical line
  doc
    .moveTo(15, yStart + 27)
    .lineTo(doc.page.width - 15, yStart + 27)
...
export const creatHealthcareChildDetail = (
  doc: InstanceType<typeof PDFDocument>,
  policy: FullPolicy,
  order: FullOrder
) => {
