...
export function selfCarePdf(doc: InstanceType<typeof PDFDocument>, policy: FullPolicy, order: FullOrder, qrImageUrl: string) {
    ...
    const creatgeneralHomecareTable2 = (doc: InstanceType<typeof PDFDocument>, policy: FullPolicy, order: FullOrder) => {
        ...
        if (beneficiaryData != null) {
            ...
            currentY += rowHeight;
            drawTableRow(doc, currentY, ["Email", "Name Beneficiary"], [customerData?.email || "", beneficiaryData.name || ""], [250, 250]);
            currentY += rowHeight;
            drawTableRow(doc, currentY, ["Date of Birth", "Relationship with Beneficiary"], [format(new Date(customerData?.dob || new Date()), "MMM dd, yyyy") || "", beneficiaryData.relation || ""], [250, 250]);
            currentY += rowHeight;
            drawTableRow(doc, currentY, ["Age", "Beneficiary's Contact No."], [customerData?.age?.toString() || "", beneficiaryData.contact_number || ""], [250, 250]);
            currentY += rowHeight;
        } else {
            drawTableRow(doc, currentY, ["Phone", "Email"], [customerData?.contact_number || "", customerData?.email || ""], [250, 250]);
            currentY += rowHeight;
            drawTableRow(doc, currentY, ["Age", "Date of Birth"], [customerData?.age?.toString() || "", format(new Date(customerData?.dob || new Date()), "MMM dd, yyyy") || ""], [250, 250]);
            currentY += rowHeight;
        }
    ...
    const creatgeneralHomecareTable3 = (doc: InstanceType<typeof PDFDocument>, policy: FullPolicy, order: FullOrder) => {
        ...
        drawTableRow(doc, currentY, ["Plan Selected", netPremium], [plan.name || "", `PKR ${new Intl.NumberFormat().format(+policy.item_price)}/-` || ""], [250, 250]);
        currentY += rowHeight;
        drawTableRow(doc, currentY, [sumInsured, periodOfInsurance], [sumValue.toString() || "", `From: ${format(new Date(policy.issue_date), "MMM dd, yyyy")}\nTo: ${format(new Date(policy.expiry_date), "MMM dd, yyyy")}` || ""], [250, 250]);
        currentY += rowHeight;
        currentY += 4;
        if (isFranchiseOrder) {
            drawTableRow(doc, currentY, ["Coverage", "Limit", ""], ["", ""], [166.6, 166.6, 166.6]);
            currentY += rowHeight;
            drawTableRow(doc, currentY, ["", "100,000", ""], ["", ""], [166.6, 166.6, 166.6]);
            currentY += rowHeight;
            drawTableRow(doc, currentY, ["Accidental Medical Expense", "25,000", ""], ["", ""], [166.6, 166.6, 166.6]);
            currentY += rowHeight;
            drawTableRow(doc, currentY, ["Burial & Repatriation", "20,000", ""], ["", ""], [166.6, 166.6, 166.6]);
            currentY += rowHeight;
            drawTableRow(doc, currentY, ["Loss of Identification Papers & Documents", "5,000", ""], ["", ""], [166.6, 166.6, 166.6]);
            currentY += rowHeight;
        }
    ...
    let productLogo = `${__dirname}../../../../assets/logo/self.png`;
    ...
    let paragraph = "All Terms, Conditions and Exclusions as per standard Jubilee General SelfCare " + policyWording + " and Clauses\n" +
        "The agreement to " + purchase + " is a declaration that I/we have read & understood the terms & conditions stated in the " + participant + " & " +
