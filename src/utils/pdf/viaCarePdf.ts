...
export function viaCarePdf(
  ...
  const creatViacareTable2 = (
    ...
    const hasPassport =
      customerData?.passport_no && customerData.passport_no.length > 0;
    if (hasPassport) {
      drawTableRow(
        doc,
        currentY,
  ...
  const creatViacareTable3 = (
    ...
    drawTableRow(
      ...
      currentY,
      ["Age", "Passport No"],
      [spouseData?.age?.toString() || "", spouseData?.passport_no || "---"],
      [250, 250]
    );
    currentY += rowHeight;
    ...
    if (spouse1Data != null && spouse1Data != undefined) {
      ...
      drawTableRow(
        ...
        currentY,
        ["Age", "Passport No"],
        [spouse1Data?.age?.toString() || "", spouse1Data.passport_no || "---"],
        [250, 250]
      );
      currentY += rowHeight;
    }
  ...
  const creatViacareTable4 = (
    ...
    const fromDate = format(
      new Date(travelData.travel_start_date || Date.now()),
      "MMM dd, yyyy"
    );
    const toDate = format(
      new Date(travelData.travel_end_date || Date.now()),
      "MMM dd, yyyy"
    );
    ...
    doc
      .moveTo(padding, yStart - 5)
      .lineTo(padding, currentY)
      .stroke(); // Left vertical
    doc
      .moveTo(doc.page.width - padding, yStart - 5)
      .lineTo(doc.page.width - padding, currentY)
      .stroke(); // Right vertical
    doc
      .moveTo(padding, currentY)
      .lineTo(doc.page.width - padding, currentY)
  ...
  const creatHealthcareChildDetail = (
    doc: InstanceType<typeof PDFDocument>,
    policy: FullPolicy,
    order: FullOrder
  ) => {
    ...
    if (kid1Data != null && kid1Data != undefined) {
      drawTableRow(
        doc,
        currentY,
        ["Name", "Relation", "Age", "Passport No"],
        [
          kid1Data.name || "",
          kid1Data.relation || "",
          kid1Data.age?.toString() || "",
          kid1Data.passport_no || "",
        ],
        [150, 120, 120, 130]
      );
      currentY += rowHeight;
  ...
  const createNewFranchiseCoverageDetails = (
    doc: InstanceType<typeof PDFDocument>,
    policy: FullPolicy,
    order: FullOrder
  ) => {
  ...
  const creatViacareTable5 = (
    ...
    drawTable(doc, [["Plan (Limits in PKR only)"]], {
      x: 15,
      y: currentY,
      columnWidths: [565],
      headers: [],
    ...
    if (
      productName
        .toLowerCase()
        .includes("viacare travel insurance – domestic plan")
    ) {
    ...
    drawTable(doc, [["* In case of Natural Death only"]], {
      x: 15,
      y: currentY,
      columnWidths: [565],
      headers: [],
  ...
  const createStudentProgramDetails = (
    doc: InstanceType<typeof PDFDocument>,
    policy: FullPolicy,
    order: FullOrder
  ) => {
    ...
    drawTableRow(
      ...
      [travelData.institute || "", travelData.program || ""],
      [250, 250]
    );
    currentY += rowHeight;
    drawTableRow(
      doc,
      currentY,
      ["Program Duration", "Offer Letter"],
      [travelData.program_duration || "", travelData.offer_letter_ref_no || ""],
      [250, 250]
    );
    currentY += rowHeight;
  ...
  const createStudentSponsersDetails = (
    doc: InstanceType<typeof PDFDocument>,
    policy: FullPolicy,
    order: FullOrder
  ) => {
  ...
  const createZiaratCoverageDetails = (
    doc: InstanceType<typeof PDFDocument>,
    policy: FullPolicy,
    order: FullOrder
  ) => {
  ...
  const addFooter = (
    ...
    if (policy.apiUser?.name.includes("bookmee")) {
    } else {
      ...
      doc
        .fontSize(8)
        .font("Helvetica")
        .text("GLOBAL RESPONSE    Tel: +27 10 100 3045", margin, yStart + 77, {
          width: doc.page.width / 2,
          align: "left",
        });
      doc.text(
  ...
  let productLogo = path.join(process.cwd(), "uploads", "logo", "travel.png");
  ...
  if (policy.takaful_policy) {
    paragraph =
      "Your Takaful does not cover any claim in any way caused by or resulting from Corona Virus itself and/ or from its fear or threat.All Terms, Conditions and Exclusions as per standard Jubilee General ViaCare Takaful wordings and Clauses\n" +
    ...
    productName
      .toLowerCase()
      .includes("via care rest of the world travel insurance")
  ) {
    paragraph =
      "Your Insurance does not cover any claim in any way caused by or resulting from Corona Virus itself and/ or from its fear or threat.This Policy covers Hospitalization and Medical Expense for USD 10,000 limit.\n" +
      "All Terms, Conditions and Exclusions as per standard Jubilee General ViaCare Policy wordings and Clauses\n" +
      "The agreement to purchase this insurance coverage is a declaration that I/we have read & understood the terms & conditions stated in the policy wordings & " +
      "clauses and I/we hereby agree to the terms, conditions & exclusions stated therein. I also declare and affirm that I am in good health I hereby declare that all " +
      "information stated in this schedule is true and complete and that I/we have not concealed any material confirmation from Jubilee General Insurance Company " +
  ...
  if (policy.schengen) {
    paragraph =
      "This Policy covers Hospitalization and Medical Expense for USD 50,000 limit, being higher than the requirement of EU30,000.\n" +
