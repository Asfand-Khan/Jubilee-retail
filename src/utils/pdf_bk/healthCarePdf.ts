...
export function healthCarePdf(
  ...
  isFaysalBankOrder =
    apiUser != null && apiUser.name.toLowerCase().includes("faysalbank");
  ...
  isHBLMFBOrder =
    apiUser != null &&
    apiUser.name.toLowerCase().includes("hbl microfinance bank");
  ...
  if (apiUser != null && apiUser.name.toLowerCase().includes("easyinsurance")) {
    ...
    apiUser != null &&
    apiUser.name.toLowerCase().includes("smartchoice")
  ) {
    isSmartChoiceOrder = true;
  } else if (
    apiUser != null &&
    apiUser.name.toLowerCase().includes("mawazna")
  ) {
    isMawazanaOrder = true;
  } else if (
    apiUser != null &&
    apiUser.name.toLowerCase().includes("lets compare")
  ) {
    isLetsCompareOrder = true;
  }

  const creatHealthcareTable2 = (
    doc: InstanceType<typeof PDFDocument>,
    policy: FullPolicy,
    order: FullOrder,
    isHMBOrder: boolean
    ...
    if (
      apiUser != null &&
      (apiUser.name.includes("faysalbank") || apiUser.name.includes("scb"))
    ) {
    ...
    if (
      productName.includes("parents-care-plus") ||
      productName.includes("parent")
    ) {
    ...
    drawTableRow(
      ...
      [
        customerData?.email || "",
        `PKR ${NUMBER_FORMAT.format(+order.payment)}/-`,
      ],
      [250, 250]
    );
    currentY += rowHeight;
    ...
    drawTableRow(
      doc,
      currentY,
      ["Date of Birth", `${policyType} Expiry Date`],
      [
        format(new Date(customerData?.dob || Date.now()), DATE_FORMAT),
        `From: ${format(new Date(policy.start_date), DATE_FORMAT)}`,
      ],
      [250, 250]
    );
    currentY += rowHeight;
    ...
    drawTableRow(
      ...
      currentY,
      [label7a, ""],
      [value7a, `To: ${format(new Date(policy.expiry_date), DATE_FORMAT)}`],
      [250, 250]
    );
    currentY += rowHeight;
    ...
    if (productName.includes("parents-care-plus")) {
      ...
      drawTableRow(
        doc,
        currentY,
        ["In Patient Price", "Out Patient Price"],
    ...
    const finalY = productName.includes("parents-care-plus")
      ? yStart + 100
      : yStart + 85;
    ...
    if (
      ...
      drawTableRow(
        ...
        [beneficiaryData.name, beneficiaryData.contact_number || ""],
        [250, 250]
      );
      currentY += rowHeight;
  ...
  const createParentCareTable = (
    ...
    const motherData = policy.policyDetails.find(
      (detail) => detail.type.toLowerCase() == "mother"
    );
    const fatherData = policy.policyDetails.find(
      (detail) => detail.type.toLowerCase() == "father"
    );
    const motherInLawData = policy.policyDetails.find(
      (detail) => detail.type.toLowerCase() == "mother-inlaw"
    );
    const fatherInLawData = policy.policyDetails.find(
      (detail) => detail.type.toLowerCase() == "father-inlaw"
    );
    const bothInLawData = policy.policyDetails.find(
      (detail) => detail.type.toLowerCase() == "both-inlaw"
    );
  ...
  const creatHealthCareRiderTable = (
    ...
    const x = 20;
    const riders =
      policy.FblPolicyRider.length > 0 ? policy.FblPolicyRider : [];

    doc
      .fontSize(10)
      .font("Helvetica-Bold")
      .text("Rider's Details", x)
      .moveDown(0.5);
    ...
    riders.forEach((rider) => {
      drawTableRow(
        doc,
        currentY,
        ["Covered Name", "Sum Assured"],
        [rider.rider_name || "", rider.sum_insured || ""],
        [250, 250]
      );
      currentY += rowHeight;
    });
  ...
  const creatHealthcareTable4 = (
    ...
    if (
      isFaysalBankOrder &&
      productName.includes("female centric health takaful")
    ) {
      ...
      drawTable(
        ...
        [
          ...
          ["Tele Health", "Sehat Kahani", "Covered"],
        ],
        {
          x: 20,
          y: currentY,
      ...
      let nursingBenefitsValue =
        "The product also provides a nursing care benefit of PKR " +
      ...
      drawTable(
        ...
        [
          ...
          [
            "Ambulance - per Hospitalization / per policy",
            `${ambulanceCharges}/-`,
          ],
          ["Pre Hospitalization", `${preHospitalizationValue}`],
          [
            "Post Hospitalization – Nursing Care Benefit: PKR 20,000 / Year",
            `${nursingBenefitsValue}`,
          ],
          [
          ...
          [
            "Pre-Existing Conditions & Congenital Anomalies Coverage",
            `${preExistingValue}`,
          ],
          [
      ...
      drawTable(
        ...
        [
          ...
          [
            "Pre-existing Condition Covered (Waiting Period)",
            "45 Days (PEC not covered for some conditions as mentioned below in major conditions & exclusions)",
          ],
          [
            "Specialized Investigations Outpatient Cover: (Magnetic Resonance Imaging (MRI), Computed Tomography (CT) Scan, Endoscopy. Thallium Scan, Angiography)",
            "Covered during hospitalization only",
          ],
          [
            "Treatment of Fractures & Lacerated Wound, Local road ambulance for emergencies only.",
            "Covered",
          ],
          [
          ...
          [
            "Loss of Income during Hospitalization (Patient should be confined within a hospital for at least 7 Consecutive days & Benefit will be payable for admitted days only of each hospitalization).",
            "1000/- per day",
          ],
      ...
      if (isFaysalBankOrder) {
        ...
        tableHeading =
          "SelfCare (Accidental Death, Disability & Medical Expense Insurance Cover)";
      ...
      if (isHBLMFBOrder) {
        ...
        drawTable(
          ...
          [
            ["Daily Cash", "Rs./Day"],
            [
              "Min 2 days & 10 days Max one Confinement & 30 days in a year\n1. Pays daily benefit for as long as the policyholder is confined to the hospital. The amount will depend on the nature of hospitalization and the mode of treatment.",
              hospitalBenefits,
            ],
            [
              "2- The benefit payable is increased by 50% if the hospitalization is due to surgery",
              surgeryBenefits,
            ],
            [
              "3- The benefit payable is increased by 50% if the hospitalization is due to accident.",
              accidentBenefits,
            ],
            [
              "4- The benefit payable is increased by 50% if the patient is confined to an ICU.",
              icuBenefits,
            ],
        ...
        drawTable(
          ...
          [
            ...
            [
              " 2. The benefit payable is increased by 50% if the hospitalization is due to surgery & accident",
              surgeryOrAccidentBenifit,
            ],
            [
              " 3. The benefit payable is increased by 100% if the patient is confined to an ICU.",
              icuBenifit,
            ],
            ["Daily Cash Max Limit in Aggregate", dailyCashMaxLimit],
            [
              "In Case of Surgery/Accident Max Limit",
              surgeryOrAccidentMaxLimit,
            ],
            ["In Case of ICU Treatment Max Limit", icuMaxLimit],
        ...
        !productName.includes("hbl") &&
        !productName.includes("shifa-daily") &&
        !productName.includes("sehat plan")
      ) {
        ...
        if (isParent) {
          ...
          subLimitRows.push([
            "Day-Care Procedures & Specialized Investigations in outpatient setting including but not limited to: Dialysis, Cataract Surgery, MRI, CT Scan, Endoscopy, Thallium Scan, Angiography, Treatment of Fracture. Emergency dental treatment due to accidental injuries within 48 hours(forpain relief only) ",
            "Covered",
          ]);
        } else {
          ...
          if (!isFranchiseOrder) {
            ...
            if (isHMBOrder) {
              internationalValue =
                "Covered Reasonable & Customary charges only for emergency hospitalization abroad";
            }
            subLimitRows.push([
        ...
        if (isSCBOrder) {
          ...
          if (isHMBOrder) {
            emergencyAccidentalValue =
              "Covered (within 48 hours of an accident only)";
          }
          subLimitRows.push([
            "Emergency Accidental Outpatient Expenses ",
            emergencyAccidentalValue,
          ...
          if (isHMBOrder) {
            emergencyDentalValue =
              "Covered (within 48 hours for pain relief only)";
          }
          subLimitRows.push([
            "Emergency Accidental Dental Expense ",
            emergencyDentalValue,
        ...
        if (isParent) {
          subLimitRows.push([
            "Pre Existing Conditions & Congenital Anomalies",
            "a. 1st Year 10% of Annual Limit.\nb. 2nd Year 20% of Annual Limit.\nc. 3rd Year 30% of Annual Limit.\nd. 4th Year & onward 50% of Annual Limit.",
          ]);
  ...
  const addVerificationAndQR = (
    ...
    let verification =
      "This is a digitally signed copy of the " +
      policyWord +
    ...
    doc
      .font("Helvetica-Bold")
      .fontSize(9)
      .text(verification, margin, yStart + 10, {
        underline: true,
      });
    ...
    doc.text(
      "Issued by " + retailBranch + " Retail Business Division",
      margin,
      yStart + 55
    );

    doc.image(qrImageUrl, doc.page.width - margin - 60, yStart, {
      width: 60,
      height: 60,
    });
    // Adjust text positions based on current y after image
    doc.text(
      `Verification code: ${policy.policy_code || "123-456-7890"}`,
      margin + columnWidth + 30,
      doc.y - 8,
      {
        underline: true,
        align: "right",
  ...
  if (
    ...
    productLogo = path.join(
      process.cwd(),
      "uploads",
      "logo",
      "parent-care-plus-icon.png"
    );
